...
def numbered(page,image,out,pno):
    ...
    for col in ((0,1) if two else (0,)):
        ...
        for i,m in enumerate(cms):
            ...
            path=save(page,image,tight(page,left,right,m.top-CAPTURE_TOP_PADDING,end),out,f"{pno:03d}p_{serial:03d}.png")
            if path: result.append(path)
    return result
...
def types(page,image,out,pno):
    ...
    for i,w in enumerate(heads):
        ...
        path=save(page,image,tight(page,55,d-5,float(w["top"])-CAPTURE_TOP_PADDING,end),out,f"{pno:03d}p_{serial:03d}_유형.png")
        if path: result.append(path)
    for i,m in enumerate(rms):
        end=rms[i+1].top-5 if i+1<len(rms) else page.height-90; serial+=1
        path=save(page,image,tight(page,d+5,page.width-55,m.top-CAPTURE_TOP_PADDING,end),out,f"{pno:03d}p_{serial:03d}_비슷한문제.png")
        if path: result.append(path)
    return result
...
def written(page,image,out,pno):
    ...
    for col in (0,1):
        ...
        for i,m in enumerate(cms):
            ...
            if not stages:
                stop=min([float(w["top"]) for w in stops] or [end]); path=save(page,image,tight(page,left,right,m.top-CAPTURE_TOP_PADDING,stop-2),out,f"{pno:03d}p_{serial:03d}.png")
                if path: result.append(path)
                continue
